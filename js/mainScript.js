$(document).ready(function() {
    ...
    $(".margin").on("change",function(){
        ...
        var mval=$(".margin").val;
        var tafterval=mval.next();
        var tbeforeval=mval.prev();
        tafterval.val(parseFloat(mval)+parseFloat(tbeforeval));
    })
    ...
    $('.add-order').on("click", function() {
        $(this).prev().append(`<tr class="accounts">
            ...
            <td><input type="text" name="spcial-comments[]" class="credit" placeholder="comment"></input></td>
    ...
    var totalval=document.querySelector(".value");
    var em=0;
    function calct(){
        for(var i=0;i<totalval.length();i++){

            $(".total-after-margin").text(
                em.innerhtml+=totalval[i]
            )
        }
    }
    calct();
